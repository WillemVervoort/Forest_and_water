...
# quick plot to show differences
Zhang_all2 %>%
...
```{r, echo =F}
pander(summary(linear_model_simple$p.table))
```
...
It is however clear from the lack of explaining power, that there could be confounding factors, as alluded to in the methods. The obvious ones being catchment dryness and area (following @zhang2017). 
...
```{r, echo = F}
pander(summary(linear_model2)$p.table)
```
...
```{r, echo =F}
pander(summary(model3_includeLatLong)$p.table)
```
...
```{r, echo =F}
pander(summary(model4_dryness)$p.table)
```
...
model5_all <- gam(DeltaQf_perc ~ DeltaF_perc_pos + Forest_Sign + s(Area_km2) + s(Dryness) + length +  Assessment_technique + Latitude ,  data = Zhang_all2 %>% filter(Dryness < 4))
summary(model5_all)
...
model6_all <- gam(DeltaQf_perc ~ DeltaF_perc_pos + Forest_Sign + te(Dryness,Latitude, k =5) + Precip_data_type + Assessment_technique + Forest_type + Hydrological_regime, data = Zhang_all2)
summary(model6_all)
...
```{r}
Zhang_all2 %>%
  ggplot(aes(Assessment_technique,Dryness)) + geom_boxplot()
```
...
<!--Check the size distribution of the catchments -->
...
```
Essentially, the analysis shows at the moment that their is no evidence that the size of the catchment influences the 
