...
#plot(model6_all)
```
```{r modelforestpos, eval =F, echo =F}
...
```
```{r modelforestnonlin, eval =F, echo =F}
...
```{r , echo = F}
pander(round(summary(Forest_model_all)$s.table,2), caption = "(\\#tab:m_all-smooth) Statistical summary for the smooth terms for the full model")
#plot(model6_all)
```
...
Inspecting the significance of the variables (Table \@ref(tab:m_all-linear) and Table \@ref(tab:m_all-smooth)) indicates some interesting features. 
...
As discussed in the methods, the single linear response to the change in forest cover was compared to a transformation of the negative forest cover to positives. This tests whether the impact on streamflow from removing forest cover is different from reforestation, as outlined in the methods. The results of this showed  
...
### Test Removal of studies of great lenght and very dry catchments
...
```{r, echo = F}
pander(round(summary(model_redLength)$p.table,2), caption = "(\\#tab:m_red-linear) Statistical summary for the linear terms the restricted model")
#plot(model6_all)
```
...
Therefore it is worth investigating what removing these few data points has on the overall model and the significance of the variables. A model testing data that have *Dryness* <= 5 and *length* <= 60 years is therefore based on a reduction of the data set from `r nrow(All_data2)` to `r nrow(NoLong_Data)` catchments.

This last model explains only slightly less of the variation with an adjusted *r^2^* of `r round(summary(model_redLength)$r.sq,2)` and a deviance explained of `r round(summary(model_redLength)$dev.expl,2)`. 

Investigating the non-linear responses suggest that *Dryness* has a non-linear response, which is significant,  where changes in forest cover in drier catchments having a greater impact on streamflow (Figure \@ref(fig:smoothsmodelredLength) and Table \@ref(tab:restrictlength)). Catchment area (*log10(Area (km^2^))*) still has an impact on flow with p = `r round(summary(model_redLength)$s.table[2,4],2)`, and the relationship visually looks almost linear. The variable *length* no longer is significant, after removal of the two studies with very long lengths.


## So far the same as zhang et al. (2017), but is there a problem?

At this point it seems like we are confirming the @zhang2017 and other results. But is there a problem hidden in the analysis?
...
```{r visualisesmoothsblabla,echo =F, eval = F}
#require(visreg)
...
dev.off()
```
```{r gamcheckredLength, echo =F, fig.cap="Residual plots for the final model indicating a small improvement in the residual distribution towards normal", out.width="90%"}
