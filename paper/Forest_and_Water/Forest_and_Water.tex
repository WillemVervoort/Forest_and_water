...
In contrast to the basic linear regression in the earlier studies (Filoso et al., 2017; Zhang et al., 2017) and the top-down Budyko modelling (Hoek van Dijke et al., 2022; Zhou et al., 2015), the regional Brazilian Cerrado study (Levy et al., 2018) provides a carefully designed statistical approach using mixed effects modelling and Differences-in-Differences modelling focusing specifically on the effect of deforestation. The analysis specifically accounted for differences between catchments and differences due to variations in climate
...
\midrule()
\endhead
\textbf{(Intercept)} & 16.53 & 5.04 & 3.28 & 0 \\
\textbf{DeltaF\_perc\_pos} & 0.34 & 0.07 & 4.56 & 0 \\
\textbf{Forest\_SignIncrease} & -48.41 & 4.92 & -9.84 & 0 \\
\textbf{Latitude} & -0.09 & 0.08 & -1.05 & 0.29 \\
\textbf{Longitude} & -0.03 & 0.03 & -1.07 & 0.28 \\
\bottomrule()
\end{longtable}

There appears to be no significant gradient in either latitude or longitude (Table \ref{tab:out-model3}), suggesting that the distribution of the catchments across the globe has little influence. The total explaining power of the model is still low with an adjusted \emph{r\textsuperscript{2}} of 0.33 suggesting further factors influencing the change in streamflow that are currently not included in the model.
...
\midrule()
\endhead
\textbf{(Intercept)} & 9.49 & 5.47 & 1.73 & 0.08 \\
\textbf{DeltaF\_perc\_pos} & 0.33 & 0.07 & 4.47 & 0 \\
\textbf{Forest\_SignIncrease} & -49.86 & 4.87 & -10.23 & 0 \\
\textbf{Dryness} & 4.66 & 2.36 & 1.97 & 0.05 \\
\bottomrule()
\end{longtable}

Similar to \emph{E0} or \emph{Pa\_mm}, the results from this model (equation \eqref{eq:eq5} and Table \ref{tab:out-model4}) interestingly indicate no impact of dryness on the change in streamflow as a function of the change in forest cover change. This might seem suprising in light of earlier reported results (Filoso et al., 2017; Zhang et al., 2017). In this case, the evidence is highly doubtful (p = 0.05). However, it is very well possible that there is a further interaction in the data with other variables or unknown variables that this simpler version of the model cannot identify. This is partly evidenced by the fact that the overall variance explained is still low, with an adjusted \emph{r\textsuperscript{2}} of 0.33. As indicated in the methods, we retain Dryness in further models as an indicator of climate for the catchments.
...
\midrule()
\endhead
\textbf{(Intercept)} & 9.96 & 6.42 & 1.55 & 0.12 \\
\textbf{DeltaF\_perc\_pos} & 0.26 & 0.09 & 2.96 & 0 \\
\textbf{Forest\_SignIncrease} & -46.37 & 5.09 & -9.12 & 0 \\
\textbf{Dryness} & 7.77 & 2.71 & 2.87 & 0 \\
\textbf{log10(Area\_km2)} & 1.35 & 2.07 & 0.65 & 0.51 \\
\textbf{Dryness:log10(Area\_km2)} & -2.56 & 1.22 & -2.09 & 0.04 \\
\bottomrule()
\end{longtable}
...
\midrule()
\endhead
\textbf{(Intercept)} & 19.11 & 5.07 & 3.77 & 0 \\
\textbf{DeltaF\_perc\_pos} & 0.27 & 0.09 & 3.12 & 0 \\
\textbf{Forest\_SignIncrease} & -51.5 & 5.79 & -8.89 & 0 \\
\bottomrule()
\end{longtable}
...
\midrule()
\endhead
\textbf{s(log10(Area\_km2))} & 1.58 & 5 & 0.73 & 0.08 \\
\textbf{s(Dryness)} & 4.62 & 9 & 1.97 & 0 \\
\textbf{s(length)} & 26.69 & 49 & 1.06 & 0 \\
\bottomrule()
\end{longtable}

Including non-linearity (Equation \eqref{eq:eq7}) increases the overall explaining power of the model to an adjusted \emph{r\textsuperscript{2}} of 0.43 and deviance explained of 0.49, but creates a few changes in the significance of the variables (Table \ref{tab:mfive-smooth}). For example, all the smoothed variables \emph{log10(Area (km\textsuperscript{2}))} (p = 0.08)), \emph{Dryness} (p = 0)) and \emph{length} (p = 0)) explain significant variation in the data.
...
\midrule()
\endhead
\textbf{(Intercept)} & 7.09 & 17.5 & 0.4 & 0.69 \\
\textbf{DeltaF\_perc\_pos} & 0.3 & 0.09 & 3.35 & 0 \\
\textbf{Forest\_SignIncrease} & -50.21 & 6.97 & -7.2 & 0 \\
\textbf{Precip\_data\_typeOB} & -15.38 & 12.75 & -1.21 & 0.23 \\
\textbf{Precip\_data\_typeSG} & -3.24 & 15.23 & -0.21 & 0.83 \\
\textbf{Assessment\_techniqueEA, HM} & 15.67 & 40.83 & 0.38 & 0.7 \\
\textbf{Assessment\_techniqueHM} & 29.29 & 11.9 & 2.46 & 0.01 \\
\textbf{Assessment\_techniquePWE} & 25.49 & 12.47 & 2.04 & 0.04 \\
\textbf{Assessment\_techniquePWE,
HM} & 12.55 & 40.66 & 0.31 & 0.76 \\
\textbf{Assessment\_techniqueQPW} & 31.72 & 19.36 & 1.64 & 0.1 \\
\textbf{Assessment\_techniqueQPW,
EA} & 38.53 & 23.04 & 1.67 & 0.1 \\
\textbf{Assessment\_techniqueSH} & 35.07 & 11.61 & 3.02 & 0 \\
\textbf{Forest\_typeCF} & -8.77 & 7.19 & -1.22 & 0.22 \\
\textbf{Forest\_typeMF} & -0.05 & 7.38 & -0.01 & 0.99 \\
\textbf{Hydrological\_regimeSD} & 5.25 & 8.71 & 0.6 & 0.55 \\
\bottomrule()
\end{longtable}
...
\midrule()
\endhead
\textbf{s(Dryness)} & 3.51 & 9 & 3.72 & 0 \\
\textbf{s(log10(Area\_km2))} & 0.83 & 4 & 1.08 & 0.02 \\
...
\midrule()
\endhead
\textbf{(Intercept)} & 12.6 & 16.34 & 0.77 & 0.44 \\
\textbf{DeltaF\_perc\_pos} & 0.24 & 0.09 & 2.81 & 0.01 \\
\textbf{Forest\_SignIncrease} & -49.62 & 6.4 & -7.76 & 0 \\
\textbf{Precip\_data\_typeOB} & -16.92 & 11.7 & -1.45 & 0.15 \\
\textbf{Precip\_data\_typeSG} & -3.4 & 13.84 & -0.25 & 0.81 \\
\textbf{Assessment\_techniqueHM} & 27.5 & 10.72 & 2.57 & 0.01 \\
\textbf{Assessment\_techniquePWE} & 22.58 & 11.56 & 1.95 & 0.05 \\
\textbf{Assessment\_techniqueQPW} & 26.66 & 18.13 & 1.47 & 0.14 \\
\textbf{Assessment\_techniqueSH} & 35.48 & 10.8 & 3.29 & 0 \\
\textbf{Forest\_typeCF} & -8.73 & 6.75 & -1.29 & 0.2 \\
\textbf{Forest\_typeMF} & 1.53 & 6.99 & 0.22 & 0.83 \\
\textbf{Hydrological\_regimeSD} & 2.6 & 8.3 & 0.31 & 0.75 \\
\bottomrule()
\end{longtable}
...
\midrule()
\endhead
\textbf{s(Dryness)} & 3.48 & 9 & 3.64 & 0 \\
\textbf{s(log10(Area\_km2))} & 0.81 & 9 & 0.47 & 0.02 \\
\textbf{s(length)} & 0 & 9 & 0 & 0.52 \\
\bottomrule()
\end{longtable}
...
Hoek van Dijke, A.J., Herold, M., Mallick, K., Benedict, I., Machwitz, M., Schlerf, M., Pranindita, A., Theeuwen, J.J.E., Bastin, J.-F., Teuling, A.J., 2022. Shifts in regional water availability due to global tree restoration. Nature Geoscience 15, 363--368. doi:\href{https://doi.org/10.1038/s41561-022-00935-0}{10.1038/s41561-022-00935-0}
...
Levy, M.C., Lopes, A.V., Cohn, A., Larsen, L.G., Thompson, S.E., 2018. Land use change increases streamflow across the arc of deforestation in brazil. Geophysical Research Letters 45, 3520--3530. doi:\url{https://doi.org/10.1002/2017GL076526}

\leavevmode\vadjust pre{\hypertarget{ref-navas2019}{}}%
Navas, R., Alonso, J., Gorgoglione, A., Vervoort, R.W., 2019. \href{https://www.mdpi.com/2073-4441/11/7/1433}{Identifying climate and human impact trends in streamflow: A case study in uruguay}. Water 11, 1433.
