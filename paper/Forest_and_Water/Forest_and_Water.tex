...
\begin{frontmatter}
  ...
  \title{Factors determining how catchments respond to forest cover change. Re-analysing global data sets.}
   ...
   \ead{jalonso@fing.edu.uy} 
    ...
    \address[INIA]{Project Manager, FPTA 357, Instituto Nacional de Investigacion Agropecuaria, INIA-Uruguay, Ruta 48 km 10, Rincon del Colorado, 90100 Canelones, Uruguay}
...
\textbf{(Intercept)} & 10.76 & 5.34 & 2.02 & 0.04 \\
\textbf{DeltaF\_perc\_pos} & 0.41 & 0.08 & 5.04 & 0 \\
\textbf{Forest\_Signincrease} & -38.03 & 5.26 & -7.23 & 0 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\textbf{(Intercept)} & 10.99 & 5.75 & 1.91 & 0.06 \\
\textbf{DeltaF\_perc\_pos} & 0.42 & 0.08 & 5.13 & 0 \\
\textbf{Forest\_Signincrease} & -38.99 & 5.54 & -7.04 & 0 \\
\textbf{Latitude} & -0.01 & 0.09 & -0.13 & 0.9 \\
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.14}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.15}}@{}}
\caption{\label{tab:out-model4} Results of the model including the dryness index
...
\midrule
\endhead
\textbf{(Intercept)} & 8.83 & 6.3 & 1.4 & 0.16 \\
\textbf{DeltaF\_perc\_pos} & 0.4 & 0.08 & 4.88 & 0 \\
\textbf{Forest\_Signincrease} & -38.26 & 5.32 & -7.19 & 0 \\
\textbf{Dryness} & 1.7 & 2.67 & 0.64 & 0.52 \\
\bottomrule
\end{longtable}

\hypertarget{the-results-from-this-model-equation-refeqeq5-and-table-reftabout-model4-interestingly-indicate-no-impact-of-dryness-modulating-the-change-in-streamflow-as-a-function-of-the-change-in-forest-cover-change-which-is-surprising-in-light-of-earlier-reported-results-zhang2017-filoso2017.-in-this-case-the-evidence-is-highly-doubtful-p-0.52.-however-it-is-very-well-possible-that-there-is-a-further-interaction-in-the-data-with-other-variables-or-unknown-variables-that-this-simpler-version-of-the-model-cannot-identify.-this-is-partly-evidenced-by-the-fact-that-the-overall-variance-explained-is-still-low-with-an-adjusted-r2-of-0.22.}{%
\section{\texorpdfstring{The results from this model (equation \eqref{eq:eq5} and Table \ref{tab:out-model4}) interestingly indicate no impact of dryness modulating the change in streamflow as a function of the change in forest cover change, which is surprising in light of earlier reported results (Filoso et al., 2017; Zhang et al., 2017). In this case, the evidence is highly doubtful (p = 0.52). However, it is very well possible that there is a further interaction in the data with other variables or unknown variables that this simpler version of the model cannot identify. This is partly evidenced by the fact that the overall variance explained is still low, with an adjusted \emph{r\textsuperscript{2}} of 0.22.}{The results from this model (equation \eqref{eq:eq5} and Table \ref{tab:out-model4}) interestingly indicate no impact of dryness modulating the change in streamflow as a function of the change in forest cover change, which is surprising in light of earlier reported results (Filoso et al., 2017; Zhang et al., 2017). In this case, the evidence is highly doubtful (p = 0.52). However, it is very well possible that there is a further interaction in the data with other variables or unknown variables that this simpler version of the model cannot identify. This is partly evidenced by the fact that the overall variance explained is still low, with an adjusted r2 of 0.22.}}\label{the-results-from-this-model-equation-refeqeq5-and-table-reftabout-model4-interestingly-indicate-no-impact-of-dryness-modulating-the-change-in-streamflow-as-a-function-of-the-change-in-forest-cover-change-which-is-surprising-in-light-of-earlier-reported-results-zhang2017-filoso2017.-in-this-case-the-evidence-is-highly-doubtful-p-0.52.-however-it-is-very-well-possible-that-there-is-a-further-interaction-in-the-data-with-other-variables-or-unknown-variables-that-this-simpler-version-of-the-model-cannot-identify.-this-is-partly-evidenced-by-the-fact-that-the-overall-variance-explained-is-still-low-with-an-adjusted-r2-of-0.22.}}

The results from this model (equation \eqref{eq:eq5} and Table \ref{tab:out-model4}) confirm that dryness is a clear confounding factor related to the change in streamflow as a function of the change in forest cover change. In this case the evidence is not very strong (p = 0.52. However, if the dryness index doubles (remembering that Dryness = 1 when E0 = Pa, so in this case E0 = 2×Pa, which is very dry), the change in runoff is \textasciitilde6\% greater. Again, overall variance explained is not very much, with an adjusted \emph{r\textsuperscript{2}} of 0.22.
\textgreater\textgreater\textgreater\textgreater\textgreater\textgreater\textgreater{} 70692a2b708e51dd658780f5487e0fb00aaf13a7
...
\midrule
\endhead
\textbf{(Intercept)} & 15.37 & 7.24 & 2.12 & 0.03 \\
\textbf{DeltaF\_perc\_pos} & 0.3 & 0.1 & 2.94 & 0 \\
\textbf{Forest\_Signincrease} & -36.61 & 5.38 & -6.8 & 0 \\
\textbf{Dryness} & 2.48 & 2.69 & 0.92 & 0.36 \\
\textbf{log10(Area\_km2)} & -3.03 & 1.67 & -1.81 & 0.07 \\
\bottomrule
\end{longtable}
...
The results of this model (Equation \eqref{eq:eq6}) indicate there is at least some evidence (p = 0.07) that there is a reduction in the effect of forest cover change on streamflow related to Area (km\textsuperscript{2}) (Table \ref{tab:out-modelArea}). In fact, the results suggests that for every additional 10 km\textsuperscript{2} in catchment size the mean change in flow reduces by 3\%. Another interesting fact to note is that with the inclusion of Area (km\textsuperscript{2}) as a variable in the model, the effect of Dryness becomes slightly more important, possibly suggesting an interaction between Dryness and Area. Including this interaction in the model (Table \ref{tab:out-modelArea-int}) results in the increased evidence (p = 0) that Dryness affects the change in flow caused by changes in forest cover and that the effect of Area is only important (p = 0.95)) as an interaction with Dryness.
...
\midrule
\endhead
\textbf{(Intercept)} & 11.95 & 7.43 & 1.61 & 0.11 \\
\textbf{DeltaF\_perc\_pos} & 0.29 & 0.1 & 2.85 & 0 \\
\textbf{Forest\_Signincrease} & -34.62 & 5.46 & -6.34 & 0 \\
\textbf{Dryness} & 5.46 & 3.1 & 1.76 & 0.08 \\
\textbf{log10(Area\_km2)} & 0.15 & 2.35 & 0.06 & 0.95 \\
\textbf{Dryness:log10(Area\_km2)} & -2.62 & 1.37 & -1.91 & 0.06 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\textbf{(Intercept)} & 13.99 & 5.68 & 2.46 & 0.01 \\
\textbf{DeltaF\_perc\_pos} & 0.33 & 0.1 & 3.51 & 0 \\
\textbf{Forest\_Signincrease} & -37.18 & 5.48 & -6.78 & 0 \\
\bottomrule
\end{longtable}
...
We now also include the remaining categorical variables (Precipitation data type, Assessment technique, Forest type and Hydrological regime) i.e.~Equation \eqref{eq:eq2}.
...
\midrule
\endhead
\textbf{(Intercept)} & -29.23 & 18.18 & -1.61 & 0.11 \\
\textbf{DeltaF\_perc\_pos} & 0.31 & 0.09 & 3.29 & 0 \\
\textbf{Forest\_Signincrease} & -23.48 & 6.84 & -3.43 & 0 \\
\textbf{Precip\_data\_typeOB} & -5.03 & 13.94 & -0.36 & 0.72 \\
\textbf{Precip\_data\_typeSG} & 19.41 & 15.38 & 1.26 & 0.21 \\
\textbf{Assessment\_techniqueEA, HM} & 19.56 & 45.02 & 0.43 & 0.66 \\
\textbf{Assessment\_techniqueHM} & 33.57 & 12.36 & 2.72 & 0.01 \\
\textbf{Assessment\_techniquePWE} & 52.79 & 11.72 & 4.5 & 0 \\
\textbf{Assessment\_techniquePWE,
HM} & 43.58 & 45.93 & 0.95 & 0.34 \\
\textbf{Assessment\_techniqueQPW} & 39.98 & 21.36 & 1.87 & 0.06 \\
\textbf{Assessment\_techniqueQPW,
EA} & 31.36 & 27.12 & 1.16 & 0.25 \\
\textbf{Assessment\_techniqueSH} & 42.63 & 12.41 & 3.44 & 0 \\
\textbf{Forest\_typeCF} & -2.33 & 8.23 & -0.28 & 0.78 \\
\textbf{Forest\_typeMF} & -10.36 & 8 & -1.3 & 0.2 \\
\textbf{Hydrological\_regimeSD} & 6.99 & 10.25 & 0.68 & 0.5 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\textbf{s(log10(Area\_km2))} & 0 & 9 & 0 & 0.37 \\
\textbf{s(Dryness)} & 3.96 & 9 & 1.68 & 0 \\
\textbf{s(length)} & 0 & 9 & 0 & 0.36 \\
\textbf{s(From)} & 8.8 & 9 & 3.64 & 0 \\
\bottomrule
\end{longtable}

This model explains more of the variance, but the improvement is marginal compared to the previous model with a adjusted \emph{r\textsuperscript{2}} of 0.34. This indicates that the categorical variables explain a limited amount of the variance. However, interesting to note from Table \ref{tab:msix-linear} that several of the assessment methods are significant. In particular Paired Watersheds experiments (PWE), Hydrological modelling (HM) and Statistical techniques (SH) are strongly significant (\(p < 0.05\)).
In this case, \emph{Area (km\^{}2)} is no longer a significant predictor, the reasons for this will be discussed later.
...
Figure \ref{fig:smoothsmodel6} highlights that the relationship between \emph{log10(Area km\textsuperscript{2})} and the change in flow is essentially linear, not significant, and does not need to be smoothed (this is the reason why using penalized smooths following Wood (2006) is useful). It still has a negative slope, indicating that in larger catchments the impact of changes in forest cover on streamflow is less than for smaller catchments. Similarly, the \emph{length} variable is not significant. However, both \emph{From} and \emph{Dryness} variables show strong non-linearity, but this does not show a clear trend due to the scatter and the distributions of the data. For example, in the case of \emph{From} the relationship is clearly strongly influenced by the two data points before 1920. Similarly, \emph{length} and \emph{Dryness} have points with high leverage.
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.10}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.14}}@{}}
\caption{\label{tab:restrictlength} Statistical summary of the smooth terms reducing dataset to studies with the study length shorter than 60 years, Dryness \textgreater{} 4 and studies commencing after 1920}\tabularnewline
\toprule
\begin{minipage}[b]{\linewidth}\centering
~
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\midrule
\endhead
\textbf{s(Dryness)} & 3.21 & 9 & 3.22 & 0 \\
\textbf{s(log10(Area\_km2))} & 0.46 & 9 & 0.11 & 0.13 \\
\textbf{s(length)} & 0 & 9 & 0 & 0.44 \\
\textbf{s(From)} & 4.06 & 9 & 1.12 & 0.05 \\
\bottomrule
\end{longtable}
...
The flexible nature of the splines means that the length variable captures some substantial variation in the data, but it is unclear what exactly is captured. The shape of the conditional response (Figure \ref{fig:smoothsmodel6}) also does not reflect the type of response highlighted in Filoso et al. (2017) and Jackson et al. (2005). One reason could be the few data points with very long data series, and very old studies (before 1920 essentially), and highly variable responses (Figure \ref{fig:smoothsmodel6}). Therefore it can be important to investigate what removing these factors has on the overall model and the significance of the variables. The next model therefore removes the following data: \emph{Dryness} \textgreater{} 4, \emph{length} \textgreater{} 60 years, \emph{From} \textgreater{} 1920. This result in a reduction of the data set from 330 to 304 catchments.
The model has more explaining power with an adjusted \emph{r\textsuperscript{2}} of 0.34. This results in Dryness now shows a clear non-linear response with drier catchments having a greater impact of changes in forest cover on streamflow (Figure \ref{fig:smoothsmodel7}). However the other 3 variables are no longer significant due to the scatter in the data, but \emph{length} and \emph{From} show some strong local non-linear behaviour.
...
There are two caveats on this explanation. The first is that there is a distinct trend in the data between \(\Delta\)Forest cover and log10(Area (km\textsuperscript{2})) (linear regression indicates an adjusted \emph{r\textsuperscript{2}} of 0.36 with a slope of -9.76) indicating that for every 10 km\textsuperscript{2} increase in catchment size on the average the recorded forest cover change is approximately 10\% lower. This is basically a result of the fact that large changes in forest cover in larger catchments are difficult to ``implement'' in an experiment.

This is also reflected in the second caveat most of the smaller catchments are ``real observed data'' using paired watershed studies, while for larger catchments, the analysis are mostly based on modelling approximations using either elasticity analysis (EA), Hydrological modelling (HM) or a combined use of statistical methods (SH) or quasi paired watershed analysis (QPW) (Figure \ref{fig:overview}). For larger catchments, these techniques all provide an approximation of the effect of forestry on streamflow rather than a direct comparison of catchments. This is a confounding factor that is not easily addressed in the regression modelling attempted here. Furthermore, the catchments analysed using EA, are concentrated in the drier end of the Dryness index scale compared to the other methods, with only the paired watershed experiment (PWE) assessment technique covering the full range of dryness indices.
...
\midrule
\endhead
\textbf{(Intercept)} & -27.03 & 18.34 & -1.47 & 0.14 \\
\textbf{DeltaF\_perc\_pos} & 0.29 & 0.1 & 2.96 & 0 \\
\textbf{Forest\_Signincrease} & -22.23 & 6.83 & -3.25 & 0 \\
\textbf{Precip\_data\_typeOB} & -11.91 & 13.85 & -0.86 & 0.39 \\
\textbf{Precip\_data\_typeSG} & 18.07 & 15.65 & 1.15 & 0.25 \\
\textbf{Assessment\_techniqueHM} & 36.48 & 12.3 & 2.97 & 0 \\
\textbf{Assessment\_techniquePWE} & 56.52 & 12.33 & 4.58 & 0 \\
\textbf{Assessment\_techniqueQPW} & 42.07 & 21.03 & 2 & 0.05 \\
\textbf{Assessment\_techniqueSH} & 48.82 & 12.33 & 3.96 & 0 \\
\textbf{Forest\_typeCF} & -4.64 & 8.18 & -0.57 & 0.57 \\
\textbf{Forest\_typeMF} & -5.22 & 8.08 & -0.65 & 0.52 \\
\textbf{Hydrological\_regimeSD} & 8.29 & 10.25 & 0.81 & 0.42 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\textbf{s(Dryness)} & 2.93 & 9 & 2.41 & 0 \\
\textbf{s(log10(Area\_km2))} & 0.45 & 9 & 0.1 & 0.14 \\
\textbf{s(length)} & 0 & 9 & 0 & 0.4 \\
\textbf{s(From)} & 3.84 & 9 & 0.66 & 0.17 \\
\bottomrule
\end{longtable}
...
More rigorous checking of data and statistical analysis results in both agreement and disagreement with older studies. It demonstrates that analysis of large databases of essentially ``aggregated data'' should be considered carefully.
...
\item
  Possibly reduced for larger watersheds, but this is only in interaction with other factors such as climate
\item
...
Stronger statements about the trends in the change in flow cannot be made until more data or better data becomes available in this area. Furthermore, the current study analyses a large global dataset. This analysis does not exclude more local and regional effects that cannot be identified in the global data.
